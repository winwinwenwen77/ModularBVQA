...
from motion_data_loader import VideoDataset_NR_SlowFast_feature, VideoDataset_LQ_SlowFast_feature, \
    VideoDataset_NR_SlowFast_feature_sparse
...
def main(config):
    ...
    ## training data   
    if config.database == 'waterloo':
        videos_dir = 'data/Waterloo_IVC_4K/'
        datainfo = 'data/waterloo_data.csv'
        trainset = VideoDataset_NR_SlowFast_feature(config.database, videos_dir, datainfo, transformations, resize)
    ...
    with torch.no_grad():
        ...
        for i, (video, mos, video_name) in enumerate(train_loader):
            ...
            if config.database == 'waterloo':
                video_name = video_name.split('/')[-3]+'_'+video_name.split('/')[-2]+'_'+video_name.split('/')[-1]
            print(video_name)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--database', type=str, default='waterloo')
    parser.add_argument('--model_name', type=str)
    parser.add_argument('--num_workers', type=int, default=0)
    parser.add_argument('--resize', type=int, default=224)
    parser.add_argument('--gpu_ids', type=list, default=None)
    parser.add_argument('--feature_save_folder', type=str, default='data/waterloo_slowfast/')
    # 1080p 2544
